import { CommonModule } from '@angular/common';
import { Component, OnInit } from '@angular/core';
import { FormsModule } from '@angular/forms';
import { MatIconModule } from '@angular/material/icon';
import { MatInputModule } from '@angular/material/input';
import { MatPaginatorModule, PageEvent } from '@angular/material/paginator';
import { Router } from '@angular/router';
import { Subject, debounceTime, distinctUntilChanged } from 'rxjs';
import { Tarracha } from '../../../../../model/produto/tarracha';
import { TarrachaService } from '../../../../../service/produto/tarracha.service';
import { NotificationService } from '../../../../../service/notification/notification.service';


@Component({
  selector: 'app-tarracha-list',
  imports: [
    CommonModule,
    MatIconModule,
    MatPaginatorModule,
    MatInputModule,
    FormsModule,
  ],
  templateUrl: './tarracha-list.component.html',
  styleUrl: './tarracha-list.component.scss'
})
export class TarrachaListComponent {
  tarrachas: Tarracha[] = [];
  loading = false;

  // variaveis de controle para a paginacao
  totalRecords = 0;
  pageSize = 10;
  page = 0;

  searchTerm: string = '';
  private searchSubject = new Subject<string>();

  constructor(
    private tarrachaService:        TarrachaService,
    private router:              Router,
    private notificationService: NotificationService
  ) {}

  ngOnInit() {
    this.setupSearchStream();
    if (this.searchTerm === '') {
      this.loadTarrachas();
    }
    this.loadCount();
  }

  loadTarrachas() {
    this.loading = true;
    this.tarrachaService.getAll(this.page, this.pageSize).subscribe({
      next: (data) => {
        this.tarrachas = data;
        this.loading = false;
      },
      error: (error) => {
        console.error('Error loading tarrachas: ', error)
        this.loading = false;
      }
    });

    console.log(this.tarrachas);
  }

  loadCount(): void {
    this.tarrachaService.count().subscribe(data => {
      this.totalRecords = data;
    })
  }
  
  navigateToCreate(): void {
    this.router.navigate(['/admin/tarrachas/create']);
  }

  navigateToEdit(id: number): void {
    this.router.navigate(['/admin/tarrachas/edit', id]);
  }

  deleteTarracha(id: number): void {
    if (confirm('Tem certeza que deseja excluir esta cor?')) {
      this.tarrachaService.delete(id).subscribe({
        next: () => {
          this.loadTarrachas();
          this.notificationService.showSuccess('Tarracha excluída com sucesso!');
        },
        error: (error) => {
          console
          .error('Não foi possível ' + id + ': ', error);
        }
      });
    }
  }

  setupSearchStream(): void {
    this.searchSubject.pipe(
      debounceTime(300), // ESPERA 300ms depois que o usuário para de digitar
      distinctUntilChanged() // Ignora se o termo digitado for o mesmo do anterior
    ).subscribe((searchTerm: string) => {
      // Quando o stream for acionado, chama a função de busca
      this.executeSearch(searchTerm);
    });
  }
  onSearchChange(novoTermo: string): void {
    // Usa o valor do ngModel para enviar ao Subject
    this.searchSubject.next(novoTermo);
  }

  executeSearch(termo: string): void {
    if (termo.trim() === '') {
      this.loadTarrachas();
      this.loadCount();
      return;
    }

    this.loading = true;
  }

  paginar(event: PageEvent): void {
    if (this.searchTerm.trim() === '') {
        this.page = event.pageIndex;
        this.pageSize = event.pageSize;
        this.loadTarrachas();
    } else {
        // Se estiver em modo de busca, podemos apenas mostrar um aviso
        console.warn('A paginação está desativada durante a busca.');
    }
  }
}
